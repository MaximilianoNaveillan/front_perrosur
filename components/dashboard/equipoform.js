...
export default function EquipoForm({ path, _class }) {
  ...
  const handleImageDataBlog = async (imageDataBlog) => {
    if (imageDataBlog) {
      const body = new FormData();
      const imgFile = dataURLtoFile(imageDataBlog, `team-blog-${key}.png`);
      body.append("file", imgFile);
      axiosFetch.post("/api/upload", body).then((_) => {
  ...
  const handleImageData = async (imageData, imageDataBlog) => {
    if (imageData) {
      const body = new FormData();
      const imgFile = dataURLtoFile(imageData, `team-index-${key}.png`);
      body.append("file", imgFile);
      axiosFetch.post("/api/upload", body).then((res) => {
        ...
        handleImageDataBlog(imageDataBlog);
      });
    } else {
      handleImageDataBlog(imageDataBlog);
    }
  };
  const putData = async (form, imageData, imageDataBlog) => {
    setLoad({
      state: true,
      status: 200,
      msg: "Cargando.",
    ...
    axiosFetch
      ...
      .then((res) => {
        ...
        handleImageData(imageData, imageDataBlog);
      })
      .catch((error) => {
        setLoad({
          state: true,
  ...
  const postData = async (form, imageData) => {
    setLoad({
      state: true,
      status: 200,
      msg: "Cargando.",
    ...
    axiosFetch
      ...
      .then((res) => {
        ...
        const body = new FormData();
        const imgFile = dataURLtoFile(imageData, `team-index-${data._id}.png`);
        body.append("file", imgFile);
        axiosFetch.post("/api/upload", body).then((resp) => {
          ...
          body.append("file", imgFile);
          axiosFetch.post("/api/upload", body).then((response) => {
            setLoad({
              state: true,
              ...
              <div className="row">
                ...
                <div className="col-6 md-12">
                  ...
                  <div className="action-textarea">
                    <button onClick={handlePushItem}>AGREGAR</button>
                  </div>
                  ...
                  <div className="action-textarea">
                    <button onClick={handlePushParagraph}>
                      AGREGAR PÁRRAFO
                    </button>
                  </div>
                </div>
