...
export default function EquipoForm({ path, _class }) {
  ...
  const handleImageData = async (imageData) => {
    if (imageData) {
      ...
      const imgFile = dataURLtoFile(imageData, `taller-blog-${key}.png`);
  ...
  const postData = async (form, imageData) => {
    ...
    axiosFetch
      ...
      .then((res) => {
        ...
        const body = new FormData();
        const imgFile = dataURLtoFile(imageData, `taller-blog-${data._id}.png`);
        body.append("file", imgFile);
                  ...
                  <div className="row">
                    <div className="col-12 mb">
                      ...
                      <CropImg
                        ...
                        key="img-upload-1"
                        contentheight="50vh"
                        url={key ? `/images/taller-blog-${key}.png` : undefined}
                      />
                    </div>
                  </div>
                </div>
