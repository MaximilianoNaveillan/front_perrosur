...
const defaulterror = {
  ...
  class: 'server-error',
  message: '¡ Error del servidor, no se puedeobtener a los usuarios !',
};

function Render({ handleDelete, setAlert, handleEdit, session }) {
        ...
        {squeleton ? (
          ...
          <div className="row cat-table">
            {render.map((item) => (
              ...
              <div
                style={{
                  display: `${session.user.email === item.email ? 'none' : ''}`,
                }}
                key={item._id}
                className="col-12 cat-row"
              >
                <div className="row cat-item">
                  ...
                  <div className="col-2 md-6 item-option">
...
function Usuarios({ session }) {
  ...
  const handleNivelUser = () => {
    const value = form.nivel === 3 ? 2 : 3;
    setForm({
      ...form,
      [`${'nivel'}`]: value,
    });
  ...
  return (
    <>
        ...
        <div className="add" onClick={() => setAdd(false)} role="presentation">
          <div className="add-card">
            <div
              ...
              role="presentation"
            >
              <h4>{form._id ? 'EDITANDO USUARIO' : 'AGREGANDO USUARIO'}</h4>
              <div className="row">
      ...
      <style jsx>{`
        ...
        input:focus + .slide {
          box-shadow: 0 0 1px ${colors.secondary};
        }
...
export const getServerSideProps = async (context) => {
  ...
  try {
    ...
    if (
      session.user.email !== 'maximilianonaveillan@gmail.com' &&
      res.nivel > 1
    )
      return {
        redirect: {
